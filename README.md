...
For live UIs, call `pluginNode.setBypassed(true)` for emergency fail-dry behavior, read `pluginNode.health` from your meter/failover loop, and listen for `latencychange` plus `transport-pressure` to monitor transport latency, deadline misses, stale output, dropped input, underruns, audio errors, and render-budget pressure.
