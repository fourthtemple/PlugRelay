...
For deck, send, or master chains, use `createLivePerformanceRackChain()` with `chain.timing`, `createLiveEffectRackChainCalibrationWindow()`, and `scheduler.updateFromChainHealth(chain.health)` so the host gets block-based live defaults, soundcheck, and compensation for one serial effects chain instead of every plugin slot individually.
