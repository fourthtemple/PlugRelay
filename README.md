...
For deck, send, or master chains, use `createLiveEffectRackChain()` with `chain.timing` and `scheduler.updateFromChainHealth(chain.health)` so the host can compensate one serial effects chain instead of every plugin slot individually.
