...
For deck, send, or master chains, use `createLivePerformanceRackChain()` with `chain.timing`, `createLiveEffectRackChainCalibrationWindow()`, and `scheduler.updateFromChainCalibration(chain.health, calibration)` so the host gets block-based live defaults, deadline-pressure snapshots, soundcheck, and calibrated compensation for one serial effects chain instead of every plugin slot individually.
