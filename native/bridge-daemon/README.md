...
- Wire preset, sample, cache, and licensing file access through the opt-in root-limited file grant foundation. Avoid ambient filesystem access from plugin workers; grant only narrow, user-approved paths where practical, and keep browser-supplied path grants behind development opt-in until a native approval picker is available.
