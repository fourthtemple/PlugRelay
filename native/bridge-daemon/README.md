...
- Complete deeper VST3 musical-control support where the SDK exposes a safe host path, including note-expression features and richer program-list behavior beyond bounded process transport context, bounded unit metadata, and bounded program-change parameter metadata, with event-count, byte-size, timing-offset, channel/note, and worker-queue limits.
