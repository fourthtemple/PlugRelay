...
- Complete deeper VST3 musical-control support where the SDK exposes a safe host path, including richer program-list behavior and text-style note-expression handling beyond bounded value-style note-expression events, bounded process transport context, bounded unit metadata, and bounded program-change parameter metadata, with event-count, byte-size, timing-offset, channel/note, and worker-queue limits.
