export class SoundBridgeAudioProcessor extends AudioWorkletProcessor {
  ...
  private postProcessBlock(blockId: number, frames: number, channels: Float32Array[]): void {
    ...
    const transfer = new Array<ArrayBuffer>(channels.length);
    for (let channelIndex = 0; channelIndex < channels.length; channelIndex += 1) transfer[channelIndex] = channels[channelIndex].buffer as ArrayBuffer;
