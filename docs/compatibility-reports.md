...
VST3 MIDI-controller event coverage is reported as `accepted`, `missing`, or `skipped-format`. An `accepted` result means the bounded control-change, pitch-bend, and channel-pressure probe batch was accepted by the worker path; it does not prove that a specific plugin exposes controller-to-parameter mapping targets. Matrix entries include path-free flags plus bounded controller event families, CC controller numbers, channels, and event-bus indexes used by the probe, including non-main bus/channel breadcrumbs when exercised.

The summary report also counts VST3 program-list metadata as `listed`, `none`, `missing`, or `skipped-format`. VST3 program-data target coverage is reported as `targeted`, `unsupported`, `no-valid-programs`, `none`, `missing`, or `skipped-format`, with path-free flags such as `program-data-unsupported`, `empty-program-list`, `invalid-program-index`, `duplicate-program-list-id`, `duplicate-program-index`, and `bounded-target`. Matrix entries include bounded counts for unsupported, undisclosed, empty, missing-program, invalid-list, invalid-index, duplicate-list-id, duplicate-index, and valid candidate program-data targets, plus VST3 program-data byte size when export succeeds. Duplicate list IDs are ambiguous restore targets and should be treated as unsupported. Use this together with `vst3ProgramData` coverage to separate plugins that expose no SDK program lists, plugins whose program lists cannot produce bounded program data, and plugins whose program-data restore path failed.
