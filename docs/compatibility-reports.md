...
The summary report counts advertised `fileGrantOperations` such as `loadPreset`, `restoreState`, `saveStateDirectory`, `loadSample`, `openCacheDirectory`, and `loadLicense`. It also reports grant-backed workflow statuses for preset/state files plus sample, cache-directory, and license-file probes when a plugin advertises those operations. Treat the advertised operation list as workflow readiness metadata and the workflow statuses as bounded probe outcomes; do not attach private preset, sample, cache, or license files to public reports.
