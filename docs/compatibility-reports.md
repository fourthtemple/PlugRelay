...
VST3 reports also include a path-free `vst3EventProfile` derived from bounded note-expression metadata. Use it to call out coverage such as `no-note-expressions`, `invalid-metadata`, `main-event-bus`, `non-main-event-bus`, `non-main-channel`, `text-expression`, `value-expression`, and multi-event-bus or multi-channel fixtures without including plugin paths, preset contents, or license data. Matrix entries include bounded note-expression counts, text/value/invalid counts, invalid route counts, associated-parameter counts, type IDs, event-bus indexes, channels, and an explicit metadata-at-limit flag so reports can describe unusual event routing or capped partial SDK metadata without the full plugin metadata payload.

VST3 MIDI-controller event coverage is reported as `accepted`, `missing`, or `skipped-format`. An `accepted` result means the bounded control-change, pitch-bend, and channel-pressure probe batch was accepted by the worker path; it does not prove that a specific plugin exposes controller-to-parameter mapping targets. Matrix entries include path-free flags plus bounded controller family counts, controller event families, CC controller numbers, channels, event-bus indexes, and invalid number/route counts used by the probe, including multi-family and non-main bus/channel breadcrumbs when exercised.
