...
Live-render coverage is reported as `within-budget`, `over-budget`, `missing-budget`, `failed`, or `missing`. It uses the bounded `renderDurationMs`, `renderBudgetMs`, and `renderBudgetExceeded` fields from the same `processAudioBlock` probe to show whether the measured render block stayed inside its block-time budget. Treat it as live-performance evidence for AU/VST effects triage, not as a universal certification.
