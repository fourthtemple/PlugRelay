...
| LV2 extensions | Extension state, worker, UI, and other extensions require host-provided feature data and callbacks. | Keep unsupported extensions disabled; enable each extension only with explicit feature structs, bounded data, ownership checks, and worker containment. |
