...
| Parameter enumeration, program metadata, presets, and automation | Plugin-controlled names/units/ids/program/preset labels and oversized automation bursts or curves can break JSON, UI, logs, or automation paths. | Cap counts and string lengths, escape text, normalize values, bound VST3 program-list and preset snapshot metadata, apply presets by daemon-listed id only, reject writes to read-only parameters, bound automation event lists and curve expansion, and enforce per-instance ownership. |
