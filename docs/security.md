...
| Plugin editor/UI hosting | Native editor code can open windows, dialogs, clipboard, drag/drop, file grants, and platform UI surfaces. | Expose bounded per-plugin `editorKinds` for host UI gating; support bounded generic parameter editor sessions now; enable native editor windows only through an explicit separate UI broker process spawned without a shell, keep broker IPC bounded by worker limits, pass only instance-attached grants to the broker, and preserve web/local host ownership checks. |
