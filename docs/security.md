...
| Native worker command timeout | 5 seconds for VST3/AU/LV2, 1.5 seconds for repo-local example workers; live `processAudioBlock.renderTimeoutMs` can set a shorter bounded per-render deadline; then the worker is rejected, terminated, and render-quarantined until instance recreation | worker IPC commands |
