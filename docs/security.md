...
| File grants | Disabled unless explicit broker roots are configured; browser-supplied paths additionally require `SOUNDBRIDGE_FILE_GRANT_ALLOW_BROWSER_PATHS=1`; 8 per session / 64 total / 10-minute TTL; paths capped at 4096 bytes, display names capped at 160 bytes | `createFileGrant`, `listFileGrants`, `revokeFileGrant` |
