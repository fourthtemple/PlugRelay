...
- optional browser worker transport that owns WebSocket, JSON, and binary audio frame encode/decode, with direct `AudioWorklet` audio ports, initial `SharedArrayBuffer` audio rings, recycled fallback input buffers, adaptive output latency, and transferred binary output buffers for live processing
...
The current worker-owned WebSocket audio path is good for correctness and demos. It now avoids the page thread, uses initial `SharedArrayBuffer` rings where browser isolation allows them, recycles fallback worklet input buffers after worker send, avoids extra binary-output cloning, and adapts output latency within bounded blocks, but it is not the final low-latency transport.

- Tune the `SharedArrayBuffer` ring path for sustained live sets, including ring-depth policy, underrun recovery, and measured jitter reporting.
