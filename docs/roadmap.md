...
- host-owned serial rack chains expose bounded chain duration, process-budget misses, repeated-miss trip status, health snapshots, and budget/retry events so DJ/live hosts can see when the combined rack misses its block deadline even if each individual stage appears healthy
