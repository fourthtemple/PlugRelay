...
- live-performance defaults for Web Audio graph effects and host-owned DJ/effects block processing, including binary audio, bounded in-flight work, shared-memory-capable transfer, adaptive latency limits, block-time input freshness and processing deadlines, wet/dry transition fades, and bounded render-pressure recovery
