...
- optional browser worker transport that owns WebSocket, JSON, and binary audio frame encode/decode, with direct `AudioWorklet` audio ports, recycled input buffers, adaptive output latency, and transferred binary output buffers for live processing
...
The current worker-owned WebSocket audio path is good for correctness and demos. It now avoids the page thread, recycles worklet input buffers after worker send, avoids extra binary-output cloning, and adapts output latency within bounded blocks, but it is not the final low-latency transport.
