...
- The website-playable VST3/AU/LV2 instruments are repo-local example bundles rendered by the native example renderer; broader MIDI event types, richer parameter automation, LV2 parameter support, advanced bus negotiation, plugin UI, and brokered filesystem access are still security-sensitive native milestones.
