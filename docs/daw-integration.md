...
- send bounded `processAudioBlock.transport` context when the host knows play state, sample position, tempo, time signature, or loop range; `liveTransportForBlock()` builds a daemon-valid block transport object and can optionally compensate sample position by reported output latency
