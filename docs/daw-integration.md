...
- apply only daemon-listed preset ids and pass native-approved preset/sample/cache/license files through opaque session-owned file grants attached to the owning plugin instance; use `useFileGrant` for compatible worker operations instead of exposing arbitrary filesystem access
...
- The website-playable VST3/AU/LV2 instruments are repo-local example bundles rendered by the native example renderer; LV2 UI declaration metadata is path-free scanner data, while LV2 UI hosting, platform-native plugin UI, advanced LV2 extension profiles, and plugin-specific VST3/AU/LV2 handlers behind the bounded `useFileGrant` worker operation are still security-sensitive native milestones.
