...
- account for `getLatency()` in timeline scheduling and monitoring UI; `SoundBridgeLiveEffectRack.refreshLatency()` and `SoundBridgeAudioNode.refreshLatency()` keep plugin, transport, and total reported latency in host-readable health
- call `SoundBridgeAudioNode.setBypassed(true)` for Web Audio graph emergency fail-dry paths instead of destroying and recreating the plugin instance during a live set; the worklet outputs dry input and discards queued or pending wet responses from before the bypass transition, and `setBypassed(false)` is the explicit retry after render-pressure or audio-error auto-bypass
- read `SoundBridgeAudioNode.health` and listen for `latencychange`, `transport-pressure`, `render-budget-exceeded`, `render-budget-auto-bypassed`, `audio-error-auto-bypassed`, and `audio-error` when using the Web Audio graph path; health includes bypass state, render pressure, render-budget auto-bypass, audio errors, audio-error auto-bypass, plugin/transport/reported latency, adaptive latency counters, deadline misses, stale output blocks, dropped input blocks, underruns, shared-audio drops, and the latest pressure/latency reasons
