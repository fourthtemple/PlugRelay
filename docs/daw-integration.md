...
This is the right policy for live sets: a failed effect should become a dry bypass, not silence and not an app crash. Binary frames, direct worklet-to-worker audio ports, `SharedArrayBuffer` rings where browser isolation allows them, recycled fallback input buffers, and bounded adaptive output latency reduce page-thread serialization work and smooth short jitter bursts. Production DJ use still needs deeper shared-memory tuning, latency calibration, and host scheduling policy.

## Current Prototype Limitations

- Main-bus and bus-indexed audio blocks use binary WebSocket frames in the reference web client. The demo opts into worker-owned WebSocket transport with a direct `AudioWorklet` audio port and `SharedArrayBuffer` rings when cross-origin isolation is available. The fallback port path recycles transferred input buffers after worker send, queues transferred binary output buffers without cloning them again, and reports bounded adaptive latency changes plus transport latency samples. JSON audio blocks remain the compatibility path for hosts that opt out of binary transport.
