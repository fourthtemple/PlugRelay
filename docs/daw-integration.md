...
- read `SoundBridgeAudioNode.health` and listen for `latencychange`, `transport-pressure`, `transport-pressure-auto-bypassed`, `render-budget-exceeded`, `render-budget-auto-bypassed`, `audio-error-auto-bypassed`, and `audio-error` when using the Web Audio graph path; health includes bypass state, transport pressure, transport-pressure auto-bypass, render pressure, render-budget auto-bypass, audio errors, audio-error auto-bypass, plugin/transport/reported latency in samples and milliseconds, render-response plugin latency changes, adaptive latency counters, deadline misses, stale output blocks, dropped input blocks, underruns, shared-audio drops, and the latest pressure/latency reasons; shared-ring input pressure and response jitter can raise bounded adaptive latency, and `statsIntervalBlocks` is bounded with the live preset reporting every 32 blocks by default
