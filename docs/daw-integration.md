...
- read `SoundBridgeAudioNode.health` and listen for `latencychange`, `transport-pressure`, `render-budget-exceeded`, and `audio-error` when using the Web Audio graph path; health includes bypass state, render pressure, audio errors, plugin/transport/reported latency, adaptive latency counters, deadline misses, stale output blocks, dropped input blocks, underruns, shared-audio drops, and the latest pressure/latency reasons
