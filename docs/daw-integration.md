...
- apply only daemon-listed preset ids and pass native-approved preset/sample/cache/license files through opaque session-owned file grants attached to the owning plugin instance; use `useFileGrant restoreState` for granted worker-native state restore and avoid arbitrary filesystem access for remaining file workflows
...
- The website-playable VST3/AU/LV2 instruments are repo-local example bundles rendered by the native example renderer; LV2 UI declaration metadata is path-free scanner data, and grant-backed worker-native state restore works through `useFileGrant restoreState`. LV2 UI hosting, platform-native plugin UI, advanced LV2 extension profiles, and non-state plugin-specific VST3/AU/LV2 file handlers remain security-sensitive native milestones.
