...
- read `SoundBridgeAudioNode.health` and listen for `render-budget-exceeded` plus `audio-error` when using the Web Audio graph path; health includes render pressure, audio errors, transport latency, deadline misses, stale output blocks, dropped input blocks, underruns, and shared-audio drops
