...
This is the right policy for live sets: a failed effect should become a dry bypass, not silence and not an app crash. Binary frames, direct worklet-to-worker audio ports, `SharedArrayBuffer` rings where browser isolation allows them, atomic worker wakeups where supported, recycled worklet buffers plus transport-worker shared-input buffers, measured response deadline lead/jitter, and bounded adaptive output latency reduce page-thread serialization work and smooth short jitter bursts. The worklet can also raise latency before dropouts when responses repeatedly arrive below the configured deadline-lead target. Production DJ use still needs deeper shared-memory tuning, latency calibration, and host scheduling policy.
