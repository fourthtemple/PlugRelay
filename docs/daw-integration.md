...
This is the right policy for live sets: a failed effect should become a dry bypass, not silence and not an app crash. Binary frames, direct worklet-to-worker audio ports, recycled input buffers, and bounded adaptive output latency reduce page-thread serialization work and smooth short jitter bursts, but production DJ use still needs shared-memory/ring-buffer transport.
