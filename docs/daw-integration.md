...
- read `SoundBridgeAudioNode.health` and listen for `transport-pressure`, `render-budget-exceeded`, and `audio-error` when using the Web Audio graph path; health includes render pressure, audio errors, transport latency, deadline misses, stale output blocks, dropped input blocks, underruns, shared-audio drops, and the latest transport-pressure reasons
