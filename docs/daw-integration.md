...
- Main-bus and bus-indexed audio blocks use binary WebSocket frames in the reference web client. The demo opts into worker-owned WebSocket transport with a direct `AudioWorklet` audio port and `SharedArrayBuffer` rings when cross-origin isolation is available. The shared path wakes the transport worker with `Atomics.waitAsync`/`notify` where supported and reports whether the shared path is using atomic or timer wakeups. The worklet reports native render duration, render-budget misses, response deadline lead, and jitter in both render blocks and samples, so hosts can tell plugin-side render pressure from transport timing. The fallback port path bounds in-flight posts at the worklet edge, and `audioRequestTimeoutMs` bounds how long direct/shared worker audio requests can hold native in-flight capacity without a response. Recycled input buffers after worker send, transferred binary output buffers without an extra clone, bounded adaptive latency changes, and transport latency samples keep the path observable. JSON audio blocks remain the compatibility path for hosts that opt out of binary transport.
