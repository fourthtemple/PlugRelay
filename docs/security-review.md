...
| AU/LV2 main bus buffers | Fixed | Installed AU and LV2 workers now consume explicit bounded bus-0 input buffers and return bus-0 `outputBuses` that mirror `channels`, so hosts see the same main-bus response shape across VST3/AU/LV2 while AU multi-output remains roadmap work. |
...
| Full plugin-hosting surface | Open roadmap | AU multi-output routing, remaining VST3 bus edge cases, LV2 port-groups and worker/UI support, plugin UI, and broader file access need feature-specific controls as they are implemented. |
