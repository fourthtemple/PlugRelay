...
| Regression coverage | Passing | `npm run smoke:security` exercises the fixes against a live daemon. Last recorded result: 114/114 checks passing. |
...
| Native worker IPC startup/line limits | Fixed | Native workers must report ready within a bounded timeout, and native/example worker stdout response lines are capped before JSON parsing; missing ready handshakes and oversized or unterminated responses reject pending commands and terminate the worker instead of stranding requests or growing daemon memory. |
