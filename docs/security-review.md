...
| Installed-plugin compatibility probe | Added | `npm run probe:installed` starts a temporary paired loopback daemon with an explicit origin allowlist and bounded request sizes so real VST3/AU/LV2 compatibility checks can be repeated without weakening the production security model. |
