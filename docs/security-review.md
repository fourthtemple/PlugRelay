...
| Preset files, samples, caches, licensing | Plugins often expect filesystem and sometimes network access. | Keep `setPreset` limited to bounded listed parameter snapshots. The daemon now has an opt-in root-limited file grant foundation, a native approval broker contract, path-free browser responses, instance-scoped file grant attachments, native-editor-broker delivery of attached grants, and no browser-supplied path mode unless explicitly enabled for development; remaining work is teaching native workers and platform UI brokers the plugin-specific operations that consume attached grant ids, while still avoiding ambient filesystem access and denying network access where the OS sandbox permits it. |
