...
| Worker diagnostic log sanitization | Fixed | Plugin-controlled worker stderr remains byte-budgeted and is escaped before daemon logging, so terminal control characters, carriage returns, and similar controls cannot forge log structure or affect operator terminals. |
