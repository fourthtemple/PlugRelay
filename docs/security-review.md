...
| AU realtime host-profile gating | Fixed | AU entries are classified with bounded `audioUnitHostProfile` metadata. Realtime main-bus units are hostable through the current worker, while AU offline effects and Apple system AUAudioMix/AUMultiSplitter entries stay visible as discovery-only plugins with bounded display reasons instead of spawning incompatible workers. |
...
| Preset files, samples, caches, licensing | Plugins often expect filesystem and sometimes network access. | Keep `setPreset` limited to bounded listed parameter snapshots. The daemon now has an opt-in root-limited file grant foundation, a native approval broker contract, path-free browser responses, instance-scoped file grant attachments, native-editor-broker delivery of attached grants, a bounded native-worker `useFileGrant` operation boundary, and grant-backed state restore/save plus worker-native preset-state load support in the reference VST3/AU/LV2 workers. Remaining work is arbitrary vendor preset formats plus sample/cache/license operations in each native worker and platform UI broker, while still avoiding ambient filesystem access and denying network access where the OS sandbox permits it. |
