...
| Preset files, samples, caches, licensing | Plugins often expect filesystem and sometimes network access. | Keep `setPreset` limited to bounded listed parameter snapshots. The daemon now has an opt-in root-limited file grant foundation, a native approval broker contract, path-free browser responses, instance-scoped file grant attachments, native-editor-broker delivery of attached grants, a bounded native-worker `useFileGrant` operation boundary, and no browser-supplied path mode unless explicitly enabled for development; remaining work is teaching each native worker and platform UI broker the plugin-specific operations it can safely implement, while still avoiding ambient filesystem access and denying network access where the OS sandbox permits it. |
