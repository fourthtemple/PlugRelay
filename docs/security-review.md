...
| Bounded VST3 unit/program metadata | Fixed | VST3 parameters expose capped SDK unit metadata where `IUnitInfo` provides it; program-change parameters are marked, and associated program-list names are exposed only as capped parameter metadata that selects programs through `setParameter`. |
...
| Full plugin-hosting surface | Open roadmap | AU offline/splitter/format-converter profiles, remaining VST3 bus/program-list/text-expression edge cases, LV2 UI and advanced extension support, plugin UI, and broader file access need feature-specific controls as they are implemented. |
