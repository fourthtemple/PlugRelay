...
| LV2 | Active `.lv2` bundle scanner with bounded TTL parsing | Basic LV2 C-ABI audio/control worker with atom MIDI delivery | Important for open-source plugin ecosystems. Public LV2 URI metadata is path-free. Extension state, worker, UI, and fuller extension support remain future work; keep GPL components out of the core. |
