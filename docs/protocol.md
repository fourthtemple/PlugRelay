...
`data` is the raw SDK program-data stream as base64. `programData` is the PlugRelay restore envelope that records the producing `pluginId`, VST3 format, listed program-list id/index, and raw data. Hosts should store `programData` for restore and treat both strings as opaque plugin-originated data.

### `setVst3ProgramData`

Restores a PlugRelay VST3 program-data envelope returned by `getVst3ProgramData`:
