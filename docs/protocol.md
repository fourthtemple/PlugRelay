...
`renderEngine` is optional diagnostics. Current values include `bundle-worker`, `bundle-executable`, `native-example`, `native-au`, `native-vst3`, `native-lv2`, and `js-fallback`. JSON arrays remain the compatibility path. For low-latency effects processing, the reference client and daemon also support a binary WebSocket frame for `processAudioBlock`: an 8-byte `SBA1` header prefix, a bounded UTF-8 JSON request/response header, and planar little-endian Float32 channel data. `binaryAudio.channels` and `binaryAudio.frames` describe the main `channels` block; optional `binaryAudio.inputBuses` and `binaryAudio.outputBuses` arrays carry bounded `{ index, channels, frames }` descriptors. Float32 blocks appear after the JSON header in this order: main channels, input buses, output buses. The JSON payload omits the duplicated sample arrays, and the decoder restores `channels`, `inputBuses`, and `outputBuses` before dispatching. The reference web client can keep WebSocket, JSON, and binary encode/decode work in a browser worker, connect live `AudioWorklet` blocks to that worker over a direct `MessagePort`, use bounded `SharedArrayBuffer` rings for worklet/worker audio when cross-origin isolation is available, fall back to recycled input buffers and transferred binary output buffers when it is not, and report bounded adaptive output latency as transport-latency samples.
